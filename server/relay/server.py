...
class TCPRelayServerICE:
    ...
    async def start(self):
        ...
        probe_server = await asyncio.start_server(
            self.handle_probe, self.host, self.probe_port
        )

        logger.info(
            f"TCP Relay Server v3.0 (ICE-Lite) listening on {self.host}:{self.port}"
        )
        ...
        await asyncio.gather(main_server.serve_forever(), probe_server.serve_forever())
    ...
    async def handle_probe(
        ...
        try:
            ...
            ts = time.time()
            logger.debug(
                f"Probe #{probe_num} from {session_id}: local={local_port} nat={nat_port}"
            )
            ...
            try:
                ...
                logger.debug(f"Error closing probe connection: {e}")
    ...
    async def handle_client(
        ...
        try:
            ...
            if prediction_mode not in ("delta", "external"):
                logger.warning(
                    f"Unknown prediction_mode '{prediction_mode}', defaulting to 'delta'"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Invalid prediction_range_extra_pct '{range_raw}', defaulting to 0.0"
                )
            ...
            if extra_ports:
                ...
                logger.info(
                    f"Peer {role} registered with {len(extra_ports)} bound ports: {extra_ports}"
                )
            ...
            async with lock:
                ...
                session[role] = peer
                logger.info(
                    f"Registered {role} for session {session_id}: {addr}, local_port={local_port}"
                )
            ...
            logger.info(
                f"Peer {role}: port_preserved={port_preserved}, needs_probing={peer.needs_probing}"
            )

            # Wait for messages (coordination happens in handle_probes_complete)
            await wait_for_peer_messages(
                peer, self.session_manager, self.max_scan_ports
            )
