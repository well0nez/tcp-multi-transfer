...
async def coordinate_multi_connections(
    session_id: str, session_manager, tcp_connections: int, max_scan_ports: int
):
    ...
    try:
        logger.info(
            f"Session {session_id}: Starting multi-connection coordination ({tcp_connections} connections)"
        )

        for conn_num in range(tcp_connections):
            logger.info(
                f"Session {session_id}: Coordinating connection {conn_num + 1}/{tcp_connections}"
            )
            ...
            while True:
                ...
                async with lock:
                    ...
                    sender_result = session_results.get("sender", {}).get(
                        conn_num
                    ) or sender.conn_results.get(conn_num)
                    receiver_result = session_results.get("receiver", {}).get(
                        conn_num
                    ) or receiver.conn_results.get(conn_num)
                ...
                if sender_has_port and receiver_has_port:
                    logger.debug(
                        f"Session {session_id}: Both peers have port for connection {conn_num}"
                    )
                    break
            ...
            await send_peer_info_for_connection(
                session_id, sender, receiver, conn_num, max_scan_ports, session_manager
            )
            ...
            while True:
                async with lock:
                    ...
                    if sender_ready and receiver_ready:
                        logger.info(
                            f"Session {session_id}: Both peers ready for connection {conn_num}"
                        )
                        break

                    if time.time() - start_wait > max_wait:
                        logger.error(
                            f"Session {session_id}: Timeout waiting for READYs (conn {conn_num})"
                        )
                        return

                await asyncio.sleep(0.1)

            # PERF: Adaptive GO delay based on measured RTT
            # Use max RTT from both peers, with minimum 0.5s and maximum 3.0s
            # GO delay = max(0.5, min(3.0, 2 * max_rtt + 0.3))
            # The 2x factor accounts for round-trip, +0.3s for processing margin
            sender_rtt = getattr(sender, "measured_rtt", 0.0) or 0.0
            receiver_rtt = getattr(receiver, "measured_rtt", 0.0) or 0.0
            max_rtt = max(sender_rtt, receiver_rtt)

            if max_rtt > 0.01:  # RTT was actually measured
                go_delay = max(0.5, min(3.0, 2 * max_rtt + 0.3))
                logger.info(
                    f"Session {session_id}: Adaptive GO delay = {go_delay:.3f}s (sender RTT={sender_rtt:.3f}s, receiver RTT={receiver_rtt:.3f}s)"
                )
            else:
                go_delay = 1.5  # Default fallback
                logger.debug(
                    f"Session {session_id}: Using default GO delay (no RTT measured)"
                )
            ...
            logger.info(
                f"Session {session_id}: GO sent for connection {conn_num + 1} at {start_at:.3f} (delay={go_delay:.2f}s)"
            )
            ...
            if not result:
                logger.error(
                    f"Session {session_id}: Timeout waiting for connection {conn_num} result"
                )
                return
...
async def coordinate_retry_connection(
    ...
    async with lock:
        ...
        if not session:
            logger.error(
                f"Session {session_id} not found for retry connection {conn_num}"
            )
            return
        ...
        if not sender or not receiver:
            logger.error(
                f"Session {session_id}: Missing sender or receiver for retry connection {conn_num}"
            )
            return
    ...
    while True:
        async with lock:
            ...
            if sender_ready and receiver_ready:
                logger.info(
                    f"Session {session_id}: Both peers ready for retry connection {conn_num}"
                )
                break

            if time.time() - start_wait > ready_timeout:
                logger.error(
                    f"Session {session_id}: Timeout waiting for READYs (retry conn {conn_num})"
                )
                return

        await asyncio.sleep(0.1)

    # PERF: Adaptive GO delay based on measured RTT (same logic as initial connection)
    sender_rtt = getattr(sender, "measured_rtt", 0.0) or 0.0
    receiver_rtt = getattr(receiver, "measured_rtt", 0.0) or 0.0
    max_rtt = max(sender_rtt, receiver_rtt)

    if max_rtt > 0.01:  # RTT was actually measured
        adaptive_delay = max(0.5, min(3.0, 2 * max_rtt + 0.3))
        logger.info(
            f"Session {session_id}: Adaptive retry GO delay = {adaptive_delay:.3f}s"
        )
    else:
        adaptive_delay = go_delay  # Use passed default

    start_at = time.time() + adaptive_delay
    go_msg = {
        "type": "go",
        "start_at": start_at,
        "connection_num": conn_num,
    ...
    logger.info(
        f"Session {session_id}: GO sent for retry connection {conn_num + 1} at {start_at:.3f} (delay={adaptive_delay:.2f}s)"
    )
...
async def wait_for_connection_result(
    ...
    while True:
        async with lock:
            ...
            if not session:
                logger.error(
                    f"Session {session_id} not found while waiting for conn {conn_num} result"
                )
                return None

            sender = session.get("sender")
            receiver = session.get("receiver")
            session_results = session.setdefault(
                "conn_results", {"sender": {}, "receiver": {}}
            )
            session_reasons = session.setdefault(
        ...
        if elapsed > 30.0 and not warned:
            logger.warning(
                f"Session {session_id}: Still waiting for conn {conn_num} results after {elapsed:.1f}s"
            )
            warned = True
...
async def send_peer_info_for_connection(
    ...
    sender_port = (
        sender.bound_ports[conn_num]
        if conn_num < len(sender.bound_ports)
        else sender.local_port
    )
    receiver_port = (
        receiver.bound_ports[conn_num]
        if conn_num < len(receiver.bound_ports)
        else receiver.local_port
    )
    ...
    logger.info(
        f"Session {session_id}: Connection {conn_num} - "
    ...
    sender_addrs = get_peer_addresses_with_prediction(
        sender, receiver, max_scan_ports, base_port=sender_nat_port
    )

    receiver_addrs = get_peer_addresses_with_prediction(
        receiver, sender, max_scan_ports, base_port=receiver_nat_port
    )

    logger.debug(
        f"Connection {conn_num}: sender_addrs={len(sender_addrs)} addresses (primary={sender_nat_port}), "
    ...
    msg_to_sender = {
        ...
        "your_role": "sender",
        "same_network": False,
        "peer_nat_analysis": receiver.nat_analysis.to_dict()
        if receiver.nat_analysis
        else None,
    ...
    msg_to_receiver = {
        ...
        "your_role": "receiver",
        "same_network": False,
        "peer_nat_analysis": sender.nat_analysis.to_dict()
        if sender.nat_analysis
        else None,
    ...
    logger.info(
        f"Session {session_id}: peer_info sent for connection {conn_num} "
...
def get_nat_port_for_local_port(peer, local_port: int) -> int:
    ...
    if analysis:
        if analysis.pattern_type == "port_preserved":
            logger.debug(
                f"Port-Preserved NAT: predicting NAT port {local_port} for local port {local_port}"
            )
            return local_port

        # External mode: NAT port is independent of local port.
        if (
            getattr(peer, "prediction_mode", "delta") == "external"
            and analysis.predicted_port
        ):
            predicted = max(1024, min(65535, int(round(analysis.predicted_port))))
            logger.info(
                f"External NAT mode: using predicted NAT port {predicted} for local port {local_port}"
            )
            return predicted

        # Delta-based prediction: NAT port follows local port with a stable offset.
        if (
            analysis.delta_min != 0
            or analysis.delta_max != 0
            or analysis.delta_median != 0
        ):
            predicted_nat = (
                local_port + analysis.delta_median + (analysis.predicted_shift or 0)
            )
        ...
        if analysis.predicted_port:
            predicted = max(1024, min(65535, int(round(analysis.predicted_port))))
            logger.info(
                f"Fallback to predicted NAT port {predicted} for local port {local_port}"
            )
            return predicted
