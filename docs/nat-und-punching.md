# How the punch is sized

This is the reasoning and the measurements behind what the tool does. The README covers
what you need to run it; this covers why it works the way it does.

Everything here was measured over a real path between a desktop behind carrier-grade NAT
and an office machine behind a port-preserving NAT, in paired runs that varied one factor
at a time. Where something is unmeasured, it says so.

## Coverage times draws
